"""
Since e is large, it might be possible to easily find d.
It's an oversimplification, but e and d can often have
an inverse relationship wrt. their size, where when one
is relatively small, then the other is relatively large.

If d is small, then we can find it using Weiner's attack:
https://en.wikipedia.org/wiki/Wiener%27s_attack

There is a python implementation of the attack already written.
On the command line:

$ pip3 install owiener

Github: https://github.com/orisano/owiener

The rest is easy :)
"""

import owiener
from Crypto.Util.number import long_to_bytes

N = 
e = 
c = 
d = owiener.attack(e, N)

if d:
    print(long_to_bytes(pow(c, d, N)).decode())
else:
    print('No luck!')
